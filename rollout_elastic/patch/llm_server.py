...
This area replaces the LB actor and re-homes the clients as recipe-owned
subclasses:
...
- ``ElasticLLMServerClient`` / ``ElasticFullyLLMServerClient``: complete
  subclasses of the native clients with the FT helpers baked into the class
  body. Subclassing (instead of ``@add``/``@patch`` on the native classes) is
  required because client *instances* cross process boundaries: the
  ``AgentLoopWorker`` receives a deserialized client whose class is resolved
  by reference in the worker process, where runtime patches applied in the
  driver/rollouter do not exist. A subclass defined in this module keeps every
  method visible on any deserialization path.
- ``LLMServerManager``: tracks a process-level ``run_id``, can assemble the
  ...
  (``spawn_replacement`` / ``_reclaim_ray_resources``). Manager objects only
  live in the rollouter process, so they are still extended through decorators.

The retry client lives under ``fault_tolerance`` and subclasses
``ElasticLLMServerClient``.
"""
...
# ---------------------------------------------------------------------------
# ElasticLLMServerClient — complete subclass, FT helpers baked into the class
# ---------------------------------------------------------------------------


class ElasticLLMServerClient(LLMServerClient):
    """FT-aware ``LLMServerClient`` defined as a real subclass.

    Every method lives in the class body, so a client instance deserialized in
    another process (AgentLoopWorker) resolves the full class via
    ``rollout_elastic.patch.llm_server`` — no reliance on the remote process
    having applied runtime patches to the native class. When FT is off, the
    acquire/release/generate paths fall back to the native behaviour
    (verl@dfc01f85) bit-exactly.
    """
    ...
    def __init__(
        ...
        super().__init__(config, servers, load_balancer_handle)
        self._run_id = run_id
        self._progress_store = progress_store
        self.max_model_len = max_model_len
        logger.warning(
            "[FT-check] %s created: module=%s pid=%s run_id=%s progress_store=%s",
            type(self).__name__,
            type(self).__module__,
            os.getpid(),
            run_id,
            progress_store is not None,
        )

    def __setstate__(self, state: dict[str, Any]) -> None:
        """Log cross-process deserialization (e.g. inside AgentLoopWorker)."""
        self.__dict__.update(state)
        logger.warning(
            "[FT-check] %s deserialized cross-process: module=%s pid=%s ft_methods=%s",
            type(self).__name__,
            type(self).__module__,
            os.getpid(),
            hasattr(type(self), "_generate_once"),
        )
    ...
    async def _generate_once(
        ...
        *,
        ...
        if not getattr(self, "_ft_call_logged", False):
            self._ft_call_logged = True
            logger.warning(
                "[FT-check] %s._generate_once: module=%s pid=%s ft=%s",
                type(self).__name__,
                type(self).__module__,
                os.getpid(),
                self._ft_enabled(),
    ...
    async def _acquire_server(self, request_id: str) -> tuple[str, ray.actor.ActorHandle]:
        if not self._ft_enabled():
            # Native path (verl@dfc01f85), kept bit-exact for the FT-off run.
...
# ---------------------------------------------------------------------------
# ElasticFullyLLMServerClient — FT + token continuation in the resume loop
# ---------------------------------------------------------------------------


class ElasticFullyLLMServerClient(ElasticLLMServerClient):
    """FT-aware ``FullyLLMServerClient`` defined as a real subclass.

    The resume loop keeps the native abort path (partial tokens kept, resume
    from ``prompt + token_ids``) and adds a fault path: ``ServerUnavailable``
    resets to the original prompt and retries on a fresh server. With mode C
    the prefix is decided by the persisted checkpoint (``create_or_resume``).
    """
...
def get_client(self, fully_async: bool = False, retry: bool = False) -> LLMServerClient:
    ...
    if fully_async:
        return ElasticFullyLLMServerClient(**common)
    if retry:
        from verl.workers.rollout.fault_tolerance.retry_client import RetryLLMServerClient

        return RetryLLMServerClient(**common)
    return ElasticLLMServerClient(config=self.config, servers=servers, load_balancer_handle=self.global_load_balancer)
...
# ---------------------------------------------------------------------------
# Import-time selfcheck: verifies the FT subclass bodies and the manager patch
# really landed in this process. One line per process that loads this module.
# ---------------------------------------------------------------------------
logger.warning(
    "[FT-check][selfcheck] llm_server: ElasticLLMServerClient(ft_methods=%s) "
    "ElasticFullyLLMServerClient(generate=%s) get_client_patched=%s pid=%d",
    all(
        hasattr(ElasticLLMServerClient, n)
        for n in (
            "_call_server",
            "_generate_once",
            "_acquire_server",
    ...
    hasattr(ElasticFullyLLMServerClient, "generate"),
    getattr(LLMServerManager.get_client, "__module__", "") == __name__,
    os.getpid(),
)
